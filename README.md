...
- `WORKDIRS`: list of paths to docker compose projects, format: *projectname:/path/to/project;...*
- `DOCKER_HOST`: docker socket path, default: *unix:///var/run/docker.sock*
- `PORT`: port to bind to, default: *8080*
- `IP`: ip address to bind to, default: *0.0.0.0*
- `DOCKER_COMPOSE_CMD_NAME`: docker compose command name, default: *docker-compose*
...
### Basic Docker Setup (Without Traefik)
...
1. Follow the full docker-compose.yml example in the repository
...
sudo systemctl enable mandok
sudo systemctl start mandok
```
...
- Only containers with `mandok=visible` labels are visible to this application

Example of managed container labels:
```yaml
services:
  myproject:
