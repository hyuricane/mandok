...
func doNewProject(c echo.Context) error {
	...
	if err != nil {
		return hxPages.NewProject(renderLayout, err).Render(c.Request().Context(), c.Response().Writer)
	}
	log.Printf("[DEBUG] new Project: %s", projectDir)
	c.Response().Header().Add("HX-Redirect", "/hx/project/"+projectName)
	c.Response().Header().Add("HX-Replace-URL", "/hx/project/"+projectName)
...
func startProject(c echo.Context) error {
	...
	c.Response().Header().Add("HX-Trigger", "updateStatus")
	return nil
}
...
func hxRedirect(c echo.Context, url string) {
	c.Response().Header().Add("HX-Redirect", url)
	c.Response().Header().Add("HX-Replace-URL", url)
}
